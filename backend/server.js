...
app.get('/acessos', authMiddleware, async (req, res) => { // SEM NADA

  const result = await sql`SELECT id, nome, cargo FROM users;`
  res.json(result)

})
app.delete('/admin/delete', authMiddleware, async (req, res) => {
  const { id } = req.body
...
app.post('/admin/create', authMiddleware, async (req, res) => {
  ...
  if (nome.length < 1 || senha.length < 1) return res.json('Usuário ou senha está vazio!')
  const existUser = await sql`SELECT * FROM users WHERE nome = ${nome}`
  if (existUser[0]) return res.json('Já existe usuário com este nome!')

  const senhaHash = await bcrypt.hash(senha, 10)

  try {
    const result = await sql`
    INSERT INTO users(nome, senha, cargo) VALUES (${nome}, ${senhaHash}, ${cargo});`
